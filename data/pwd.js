...
function getForms() {
  var _isUsernameFieldType = function(element) {
  ...
  if (!forms) {
    console.log("handleClick: no forms found");
    return;
  }
  if (forms.length < 1) {
    console.log("handleClick: NO forms found");
...
(function handleClick() {
  ...
  try {
  pwdForms = getForms();
  if (pwdForms.length<1) return;

  oldInnerHTML = window.document.body.innerHTML;

  window.document.body.innerHTML = '<div style="width:100%; height:100%; position:fixed; left:0pt; top:0pt; z-index:9000;">'+
      '<div id="text">Please click the QR-Code after your wallet has read it</div>' +
      '<div style="width:400; height:400; border-width:5px; border-style:solid; border-color:white;">' +
          '<canvas id="qr-code" width="250" height="250" style="display:block; width:250; height:250; margin-left: auto; margin-right: auto; border-width:5px solid white;"></canvas>' +
      '</div>' +
      '<video id="video" width="250" height="250" style="display:none; width:250; height:250; border: 5px solid red;"></video>' +
      '</div>';
  onload();

  console.log("found password input field user.name=" + pwdForms[0].user.name + " pwd.name=" + pwdForms[0].pwd.name + " form.action=" + pwdForms[0].form.action);
  var message = {}; 
  message.action = pwdForms[0].form.action;
  message.pwd = pwdForms[0].pwd.name;
  message.user = pwdForms[0].user.name;
  foundPasswordForm(message);
  } catch(e) {
    console.log("handleClick threw: " + e); 
  }
})();
...
qrcode.decode = function(src){
  ...
  {
      var canvas_qr = document.getElementById("qr-code");
...
function stopVideo() {
	try {
		...
		gHaveUserMedia = false;
	} catch(e) {
		console.error("stopVideo threw: " + e);
	}
}
...
{
	try {
		console.log("qrcode read: " + a);
		try {
            stopVideo();
	    } catch(e) {
		    console.error("qrcode read stopVideo threw: " + e);
        }
        window.document.body.innerHTML = oldInnerHTML;
        getForms();
        if (pwdForms.length > 0) {
		    console.error("qrcode read found form pwdForms[0].user.name="+pwdForms[0].user.name+" pwdForms[0].pwd.name="+pwdForms[0].pwd.name);
            var credentials = JSON.parse(a);
            pwdForms[0].user.value = credentials[0];
            pwdForms[0].pwd.value = credentials[1];
            pwdForms[0].form.submit();
        } else {
		    console.error("qrcode read oops no forms found");
        }
	} catch(e) {
		console.error("qrcode read threw: " + e);
		alert("qrcode read threw: " + e);
	}
}
...
{
	gCanvas = document.getElementById("qr-code");
	var w = ww;
	var h = hh;
	...
	imageData = gCtx.getImageData( 0,0,250,250);
}
...
function load()
{
  ...
  try {
    ...
    console.log("vor getUserMedia");
    navigator.getUserMedia({video: true, audio: false}, success, error);
    console.log("nach getUserMedia");
  } catch(e) {
...
function foundPasswordForm(obj) {
  ...
  if (obj) {
    ...
    canvas.addEventListener("click", buttonclickok, false);
    window.qr.canvas(
      { canvas: canvas, size: 10,
...
function buttonclickok(event) {
  "use strict";
  try {
	  video = document.querySelector('video');
      video.style.display = "none";
	  console.log("buttonclick ok event: " + JSON.stringify(event));
	  var canvas = document.getElementById('qr-code');
	  canvas.style.display = "";
	  load();
  } catch(e) {
  	console.error("buttonclickcancel threw " + e);
  }	  
}
function buttonclickcancel(event) {
	"use strict";
    try {
  	  console.log("buttonclick cancel event: " + JSON.stringify(event));
	  video = document.querySelector('video');
	  video.style.display = 'none';
	  var canvas = document.getElementById('qr-code');
	  canvas.style.display = "";
	  stopVideo();
      if (oldInnerHTML && oldInnerHTML != "") {
          window.document.body.innerHTML = oldInnerHTML;
      }
...
function onload() {
    ...
    if (text) {
		text.addEventListener("click", buttonclickok, false);
        text.innerText = "text";
    }
	var ok = document.getElementById("qr-code");
	if (ok) {
        console.log("setting click handler to qr-code");
		ok.addEventListener("click", buttonclickok, false);
        ok.width = "250px";
        ok.height = "250px";
        ok.style = "display:block; width:250; height:250; margin-left: auto; margin-right: auto; border-width:5px solid white;";
	} else {
		console.error("internal error: button with id=ok not found");
	}
	video = document.querySelector('video');
	if (video) {
		video.addEventListener("click", buttonclickcancel);
	} else {
		console.error("internal error: video element not found");
	}
}
